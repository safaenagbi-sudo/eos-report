...
and 20 candidate parameter combinations, totalling 100 fits per model.
Tuning is conducted on the training set only; the test set remains
unused until the final evaluation.
...
inference. The tuning results and best parameter sets for all three models
are saved to \texttt{artifacts/hyperparameter\_tuning\_results.csv}.
...
Table~\ref{tab:hyperparams} summarises the hyperparameters used for each
classical model at two stages: the \textbf{benchmark configuration},
which is the fixed starting point used during cross-validation
benchmarking and imbalance strategy selection, and the
\textbf{tuned configuration}, which is the best parameter set found by
\texttt{RandomizedSearchCV} for the three top-benchmark models. Gradient
Boosting and Linear SVM were not included in the tuning stage as they
did not rank in the top~3 by cross-validated macro F1.
...
Boosting and Linear SVM are benchmark-only.}
\label{tab:hyperparams}
\small
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Model} & \textbf{Parameter} & \textbf{Benchmark default} & \textbf{Tuned value} \\
\midrule
...
\paragraph{ROC-AUC.}
The one-vs-rest macro-averaged Area Under the Receiver Operating
Characteristic curve (\texttt{roc\_auc\_score}, \texttt{multi\_class="ovr"},
\texttt{average="macro"}) is computed using the model's predicted class
probabilities. ROC-AUC summarises the classifier's ranking ability across
all classification thresholds. For class-imbalanced datasets, however,
ROC-AUC can be misleadingly optimistic for minority classes, as it is
dominated by the large number of true negatives. It is reported for
completeness but is not the primary evaluation metric.

\paragraph{Precision--Recall AUC (PR-AUC).}
The macro-averaged Area Under the Precision--Recall curve is computed
per class and averaged across classes. Unlike ROC-AUC, PR-AUC does not
reward a high true-negative rate and is therefore more informative for
minority-class performance in imbalanced settings. A high ROC-AUC
combined with a substantially lower PR-AUC for the same class indicates
that the model's apparent discriminative power on that class is partially
an artefact of the class imbalance rather than genuine classification
quality.
