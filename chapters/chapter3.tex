...
Following final model selection, class-specific confidence thresholds were
calibrated using the held-out labelled set. Standard classifier output
assigns each QUADKEY the class with the highest predicted probability,
regardless of whether that probability is high or low. In a four-class
imbalanced setting, this can lead to low-confidence predictions being
accepted. Per-class thresholding addresses this by requiring the predicted
probability for a class to exceed a class-specific minimum before the label
is confirmed.

For each class, the model's predicted probabilities on the held-out set
were used to compute a one-vs-rest precision--recall curve. Candidate
thresholds were obtained from the predicted probability scores. For each
threshold, the per-class F1 score was computed, and the threshold that
maximized F1 was selected. The resulting thresholds were saved to
\texttt{artifacts/per\_class\_thresholds.json} and later loaded during
district-scale inference.

During inference, the selected thresholds were applied using the following
rule:
...
where $\hat{p}_{i,c}$ is the predicted probability assigned to class
$c$ for observation $i$, and $\tau_c$ is the calibrated confidence
...
loaded from \texttt{artifacts/per\_class\_thresholds.json}, where
$\hat{p}_{i,c}$ is the predicted probability assigned to class $c$ for
observation $i$. If the
